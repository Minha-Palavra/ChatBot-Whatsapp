...
export class MaterialsPurchaseDetailsState extends MessageState {
  public prefix = 'MATERIALS_PURCHASE_DETAILS';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      materialsPurchaseDetails: null,
      state: TicketState.MATERIALS_PURCHASE_DETAILS_DESCRIPTION,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.MATERIALS_PURCHASE_DETAILS_DESCRIPTION_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.MATERIALS_PURCHASE_DETAILS_DESCRIPTION
      ) {
        ticket.materialsPurchaseDetails = message.text.body;
        ...
        if (!ticket.materialsPurchaseDetails) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.MATERIALS_PURCHASE_DETAILS_DESCRIPTION_CONFIRMATION_REQUEST(),
          this.prefix,
          true,
        );
        continue;
