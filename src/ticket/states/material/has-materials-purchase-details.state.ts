...
import { MaterialsPurchaseDetailsState } from './materials-purchase-details.state';
import { HasPaymentFeeState } from '../fees/has-payment-fee-state';

export class HasMaterialsPurchaseDetailsState extends MessageState {
  public prefix = 'HAS_MATERIALS_PURCHASE_DETAILS';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      state: TicketState.HAS_MATERIALS_PURCHASE_DETAILS,
    });

    await this.whatsAppService.sendConfirmationOptions(
      phoneNumber,
      messages.HAS_MATERIALS_PURCHASE_DETAILS_REQUEST(),
      this.prefix,
      true,
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.HAS_MATERIALS_PURCHASE_DETAILS
      ) {
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new MaterialsPurchaseDetailsState();
        this.nextState.whatsAppService = this.whatsAppService;
        this.nextState.logger = this.logger;
        this.nextState.userService = this.userService;
