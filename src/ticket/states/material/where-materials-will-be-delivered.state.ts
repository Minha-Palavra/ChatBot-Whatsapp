...
import { MaterialsAreRefundableState } from './materials-are-refundable-state';

export class WhereMaterialsWillBeDeliveredState extends MessageState {
  public prefix = 'WHERE_MATERIALS_WILL_BE_DELIVERED';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      whereMaterialsWillBeDelivered: null,
      state: TicketState.WHERE_MATERIALS_WILL_BE_DELIVERED,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.WHERE_MATERIALS_WILL_BE_DELIVERED_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.WHERE_MATERIALS_WILL_BE_DELIVERED
      ) {
        ticket.whereMaterialsWillBeDelivered = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.WHERE_MATERIALS_WILL_BE_DELIVERED_CONFIRMATION,
        });

        if (!ticket.whereMaterialsWillBeDelivered) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.WHERE_MATERIALS_WILL_BE_DELIVERED_CONFIRMATION_REQUEST(
            ticket.whereMaterialsWillBeDelivered,
          ),
          this.prefix,
          true,
        );
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new MaterialsAreRefundableState();
