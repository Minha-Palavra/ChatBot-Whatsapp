...
import { WhoWillPayMaterialsState } from './who-will-pay-materials.state';

export class MaterialsHavePreDeterminedValueState extends MessageState {
  public prefix = 'MATERIALS_HAVE_PRE_DETERMINED_VALUE';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      materialsPreDeterminedValue: null,
      state: TicketState.MATERIALS_HAVE_PRE_DETERMINED_VALUE,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.MATERIALS_HAVE_PRE_DETERMINED_VALUE_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.MATERIALS_HAVE_PRE_DETERMINED_VALUE
      ) {
        ticket.materialsPreDeterminedValue = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.MATERIALS_HAVE_PRE_DETERMINED_VALUE_CONFIRMATION,
        });

        if (!ticket.materialsPreDeterminedValue) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.MATERIALS_HAVE_PRE_DETERMINED_VALUE_CONFIRMATION_REQUEST(),
          this.prefix,
          true,
        );
        continue;
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new WhoWillPayMaterialsState();
