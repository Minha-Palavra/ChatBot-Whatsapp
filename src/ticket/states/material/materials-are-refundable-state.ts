...
import { HasMaterialsPurchaseDetailsState } from './has-materials-purchase-details.state';

export class MaterialsAreRefundableState extends MessageState {
  public prefix = 'MATERIALS_ARE_REFUNDABLE';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      materiaAreRefundableState: null,
      state: TicketState.MATERIALS_ARE_REFUNDABLE,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.MATERIALS_ARE_REFUNDABLE_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.MATERIALS_ARE_REFUNDABLE
      ) {
        ticket.materiaAreRefundableState = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.MATERIALS_ARE_REFUNDABLE_CONFIRMATION,
        });

        if (!ticket.materiaAreRefundableState) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.MATERIALS_ARE_REFUNDABLE_CONFIRMATION_REQUEST(
            ticket.materiaAreRefundableState,
          ),
          this.prefix,
          true,
        );
      ...
      if (
        ...
        ticket.state !== TicketState.MATERIALS_ARE_REFUNDABLE_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new HasMaterialsPurchaseDetailsState();
