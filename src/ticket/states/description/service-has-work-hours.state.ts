...
import { ServiceWorkHoursDescriptionState } from './service-work-hours-description.state';
import { PaymentAmountState } from '../payment/payment-amount.state';

export class ServiceHasWorkHoursState extends MessageState {
  public prefix = 'SERVICE_HAS_WORK_HOURS';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      state: TicketState.SERVICE_HAS_WORK_HOURS,
    });

    await this.whatsAppService.sendConfirmationOptions(
      phoneNumber,
      messages.SERVICE_HAS_WORK_HOURS_REQUEST(),
      this.prefix,
      true,
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.SERVICE_HAS_WORK_HOURS
      ) {
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new ServiceWorkHoursDescriptionState();
        this.nextState.whatsAppService = this.whatsAppService;
        this.nextState.logger = this.logger;
        this.nextState.userService = this.userService;
        // go to the next state.
