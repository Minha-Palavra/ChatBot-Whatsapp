...
import { ServiceHasWorkHoursState } from './service-has-work-hours.state';

export class ServiceEndDateState extends MessageState {
  public prefix = 'SERVICE_END_DATE';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      serviceEndDate: null,
      state: TicketState.SERVICE_END_DATE,
    });
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.SERVICE_END_DATE
      ) {
        ticket.serviceEndDate = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.SERVICE_END_DATE_CONFIRMATION,
        });

        if (!ticket.serviceEndDate) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.SERVICE_END_DATE_CONFIRMATION_REQUEST(ticket.serviceEndDate),
          this.prefix,
          true,
        );
        continue;
      ...
      if (
        ...
        ticket.state !== TicketState.SERVICE_END_DATE_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new ServiceHasWorkHoursState();
