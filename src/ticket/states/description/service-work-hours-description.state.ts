...
import { PaymentAmountState } from '../payment/payment-amount.state';

export class ServiceWorkHoursDescriptionState extends MessageState {
  public prefix = 'SERVICE_WORK_HOURS_DESCRIPTION';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      serviceWorkHoursDescription: null,
      state: TicketState.SERVICE_WORK_HOURS_DESCRIPTION,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.SERVICE_WORK_HOURS_DESCRIPTION_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.SERVICE_WORK_HOURS_DESCRIPTION
      ) {
        ticket.serviceWorkHoursDescription = message.text.body;
        ...
        if (!ticket.serviceWorkHoursDescription) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.SERVICE_WORK_HOURS_DESCRIPTION_CONFIRMATION_REQUEST(),
          this.prefix,
          true,
        );
        continue;
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new PaymentAmountState();
