...
export class ServiceAddressState extends MessageState {
  public prefix = 'SERVICE_ADDRESS';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      serviceAddress: null,
      state: TicketState.SERVICE_ADDRESS,
    });
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.SERVICE_ADDRESS
      ) {
        ticket.serviceAddress = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.SERVICE_ADDRESS_CONFIRMATION,
        });
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.SERVICE_ADDRESS_CONFIRMATION_REQUEST(ticket.serviceAddress),
          this.prefix,
          true,
        );
      ...
      if (
        ...
        ticket.state !== TicketState.SERVICE_ADDRESS_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
