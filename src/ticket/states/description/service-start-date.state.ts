...
import { ServiceEndDateState } from './service-end-date.state';

export class ServiceStartDateState extends MessageState {
  public prefix = 'SERVICE_START_DATE';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...
      state: TicketState.SERVICE_START_DATE,
    });
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.SERVICE_START_DATE
      ) {
        ticket.serviceStartDate = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.SERVICE_START_DATE_CONFIRMATION,
        });

        if (!ticket.serviceStartDate) {
      ...
      if (
        ...
        ticket.state !== TicketState.SERVICE_START_DATE_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new ServiceEndDateState();
