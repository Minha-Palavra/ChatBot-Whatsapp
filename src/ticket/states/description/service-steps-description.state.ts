...
import { ServiceAddressState } from './service-address.state';

export class ServiceStepsDescriptionState extends MessageState {
  public prefix = 'SERVICE_STEPS_DESCRIPTION';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      serviceStepsDescription: null,
      state: TicketState.SERVICE_STEPS_DESCRIPTION,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.SERVICE_STEPS_DESCRIPTION_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.SERVICE_STEPS_DESCRIPTION
      ) {
        ticket.serviceStepsDescription = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.SERVICE_STEPS_DESCRIPTION_CONFIRMATION,
        });

        if (!ticket.serviceStepsDescription) {
      ...
      if (
        ...
        ticket.state !== TicketState.SERVICE_STEPS_DESCRIPTION_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new ServiceAddressState();
