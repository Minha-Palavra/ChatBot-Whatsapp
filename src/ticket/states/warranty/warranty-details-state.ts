...
export class WarrantyDetailsState extends MessageState {
  public prefix = 'WARRANTY_DETAILS';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...
      state: TicketState.WARRANTY_DETAILS,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.WARRANTY_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.WARRANTY_DETAILS
      ) {
        ticket.warrantyDescription = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.WARRANTY_DETAILS_CONFIRMATION,
        });

        if (!ticket.warrantyDescription) {
