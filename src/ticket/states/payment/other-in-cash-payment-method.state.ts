...
export class OtherInCashPaymentMethodState extends MessageState {
  public prefix = 'OTHER_IN_CASH_PAYMENT_METHOD';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      state: TicketState.OTHER_IN_CASH_PAYMENT_METHOD_DESCRIPTION,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.OTHER_IN_CASH_PAYMENT_METHOD_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.OTHER_IN_CASH_PAYMENT_METHOD_DESCRIPTION
      ) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.OTHER_IN_CASH_PAYMENT_METHOD_CONFIRMATION_REQUEST(
