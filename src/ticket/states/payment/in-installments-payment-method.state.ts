...
import { OtherInInstallmentsPaymentMethodState } from './other-in-installments-payment-method.state';
import { InstallmentCountState } from './installment-count.state';

export class InInstallmentsPaymentMethodState extends MessageState {
  public prefix = 'IN_INSTALLMENTS_PAYMENT_METHOD';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      paymentMethod: null,
      state: TicketState.IN_INSTALLMENTS_PAYMENT_METHOD,
    });

    await this.whatsAppService.sendPaymentInInstallmentsOptions(
      phoneNumber,
      messages.IN_INSTALLMENTS_PAYMENT_METHOD_REQUEST(),
      this.prefix,
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.IN_INSTALLMENTS_PAYMENT_METHOD
      ) {
      ...
      if (selectedOption === `${this.prefix}-CREDIT-CARD`) {
        await this.whatsAppService.ticketService.save({
          ...ticket,
          paymentMethod: 'Cartão de Crédito',
        });

        this.nextState = new InstallmentCountState();
        ...
        //
        selectedOption === `${this.prefix}-BANK-SLIP`
      ) {
        await this.whatsAppService.ticketService.save({
          ...ticket,
        ...
        this.nextState = new OtherInInstallmentsPaymentMethodState();
        this.nextState.whatsAppService = this.whatsAppService;
        this.nextState.logger = this.logger;
        this.nextState.userService = this.userService;
