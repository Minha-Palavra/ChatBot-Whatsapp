...
import { WhatIsContractCancellationDetailsState } from './what-is-contract-cancellation-details.state';

export class WhatIsContractCancellationState extends MessageState {
  public prefix = 'WHAT_IS_CONTRACT_CANCELLATION';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      whatIsContractCancellation: null,
      state: TicketState.WHAT_IS_CONTRACT_CANCELLATION,
    });
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.WHAT_IS_CONTRACT_CANCELLATION
      ) {
        ...
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.WHAT_IS_CONTRACT_CANCELLATION_CONFIRMATION,
        });

        if (!ticket.whatIsContractCancellation) {
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new WhatIsContractCancellationDetailsState();
