...
export class WhatIsContractCancellationDetailsState extends MessageState {
  public prefix = 'WHAT_IS_CONTRACT_CANCELLATION_DETAILS';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      whatIsContractCancellationDetails: null,
      state: TicketState.WHAT_IS_CONTRACT_CANCELLATION_DETAILS,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.WHAT_IS_CONTRACT_CANCELLATION_DETAILS_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.WHAT_IS_CONTRACT_CANCELLATION_DETAILS
      ) {
        ticket.whatIsContractCancellationDetails = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.WHAT_IS_CONTRACT_CANCELLATION_DETAILS_CONFIRMATION,
        });

        if (!ticket.whatIsContractCancellationDetails) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.WHAT_IS_CONTRACT_CANCELLATION_DETAILS_CONFIRMATION_REQUEST(),
          this.prefix,
          true,
        );
        continue;
