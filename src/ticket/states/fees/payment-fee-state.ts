...
export class PaymentFeeState extends MessageState {
  public prefix = 'PAYMENT_FEE';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      paymentFee: null,
      state: TicketState.PAYMENT_FEE,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.PAYMENT_FEE_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (message.type === 'text' && ticket.state === TicketState.PAYMENT_FEE) {
        ticket.paymentFee = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.PAYMENT_FEE_CONFIRMATION,
        });

        if (!ticket.paymentFee) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.PAYMENT_FEE_CONFIRMATION_REQUEST(ticket.paymentFee),
          this.prefix,
          true,
        );
        continue;
      ...
      if (
        ...
        ticket.state !== TicketState.PAYMENT_FEE_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
