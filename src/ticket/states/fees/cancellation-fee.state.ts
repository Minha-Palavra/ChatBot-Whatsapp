...
export class CancellationFeeState extends MessageState {
  public prefix = 'CANCELLATION_FEE';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...
      state: TicketState.CANCELLATION_FEE,
    });

    await this.whatsAppService.sendMessage(
      phoneNumber,
      messages.CANCELLATION_FEE_REQUEST(),
    );
  }
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.CANCELLATION_FEE
      ) {
        ticket.cancellationFee = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.CANCELLATION_FEE_CONFIRMATION,
        });
        if (!ticket.cancellationFee) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.CANCELLATION_FEE_CONFIRMATION_REQUEST(
      ...
      if (
        ...
        ticket.state !== TicketState.CANCELLATION_FEE_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
