...
import { HasCancellationFeeState } from './has-cancellation-fee.state';

export class ServiceDeliveryState extends MessageState {
  public prefix = 'SERVICE_DELIVERY';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...ticket,
      serviceDeliveryDescription: null,
      state: TicketState.SERVICE_DELIVERY,
    });
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (message.type === 'text' && TicketState.SERVICE_DELIVERY) {
        ticket.serviceDeliveryDescription = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.SERVICE_DELIVERY_CONFIRMATION,
        });

        if (!ticket.serviceDeliveryDescription) {
        ...
        await this.whatsAppService.sendConfirmationOptions(
          phoneNumber,
          messages.SERVICE_DELIVERY_CONFIRMATION_REQUEST(),
          this.prefix,
          true,
        );
        continue;
      ...
      if (
        ...
        ticket.state !== TicketState.SERVICE_DELIVERY_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new HasCancellationFeeState();
