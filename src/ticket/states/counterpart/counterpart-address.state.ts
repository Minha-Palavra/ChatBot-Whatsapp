...
import { ServiceCategoryState } from '../description/service-category.state';

export class CounterpartAddressState extends MessageState {
  public prefix = 'COUNTERPART_ADDRESS';
  ...
  public async onStateBegin(
    ...
    await this.whatsAppService.ticketService.save({
      ...
      state: TicketState.COUNTERPART_ADDRESS,
    });
  ...
  public async processMessages(value: ValueObject): Promise<void> {
    ...
    for (const message of value.messages) {
      ...
      if (
        message.type === 'text' &&
        ticket.state === TicketState.COUNTERPART_ADDRESS
      ) {
        ticket.counterpartAddress = message.text.body;

        // Update the user state.
        await this.whatsAppService.ticketService.save({
          ...ticket,
          state: TicketState.COUNTERPART_ADDRESS_CONFIRMATION,
        });

        if (!ticket.counterpartAddress) {
      ...
      if (
        ...
        ticket.state !== TicketState.COUNTERPART_ADDRESS_CONFIRMATION
      ) {
        await this.whatsAppService.sendMessage(
          phoneNumber,
      ...
      if (selectedOption === `${this.prefix}-NO`) {
        ...
        this.nextState = new ServiceCategoryState();
